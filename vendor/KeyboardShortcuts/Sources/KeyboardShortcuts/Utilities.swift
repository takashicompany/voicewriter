...
extension String {
	...
	*/
	var localized: String {
		NSLocalizedString(self, bundle: .voicewriterKeyboardShortcutsResources, comment: self)
	}
}

// MARK: - Voicewriter fork patch
//
// 背景(2026-07-24調査): 配布したVoicewriter-v0.3.0.zipで「設定→ショートカット」タブを開くと
// 必ずクラッシュする不具合が発生した。原因は本来のSwiftPM生成コード`Bundle.module`
// (resource_bundle_accessor.swift、ビルドのたびに自動生成されコミット対象外)にある:
//
//   let mainPath = Bundle.main.bundleURL.appendingPathComponent("KeyboardShortcuts_KeyboardShortcuts.bundle").path
//   let buildPath = "<ビルドマシンの.build配下への絶対パス>/KeyboardShortcuts_KeyboardShortcuts.bundle"
//   guard let bundle = Bundle(path: mainPath) ?? Bundle(path: buildPath) else { fatalError(...) }
//
// `Bundle.main.bundleURL`は.appバンドルの"直下"(Contentsと同階層)を指すため、
// 通例のContents/Resources/配置ではmainPathは見つからない。buildPathはビルドを行った
// 開発者本人のマシンにしか存在しない絶対パスのため、配布先のマシンでは両方とも見つからず
// `fatalError`でクラッシュする(実機で再現確認済み、開発機では`.build`が残っているため
// buildPathのフォールバックが偶然成立し再現しなかった)。
//
// なお「.appバンドル直下にリソースバンドルを置く」workaroundも検討したが、macOSの.appは
// "Contents/"配下のみが正規の内容物であり、`codesign`は直下(Contents外)の内容物を
// 「unsealed contents present in the bundle root」として署名を拒否するため採用できなかった
// (`scripts/build-app.sh`で実機検証済み)。
//
// そのため、上流ライブラリで唯一`Bundle.module`を参照している本ファイルの`.localized`のみ、
// 自前のフォールバック探索ロジック(`.voicewriterKeyboardShortcutsResources`)に差し替える。
// `Bundle.module`自体はどこからも参照されなくなるため(Swiftの`static let`は遅延初期化のため)、
// 生成コード自体は残っても評価されずクラッシュしない。
//
// 探索順序:
//   1. `Contents/Resources/`配下 (本アプリの`scripts/build-app.sh`が実際に配置する場所)
//   2. `Bundle.main.bundleURL`直下 (上流オリジナルのmainPath。将来配置方法を変えても壊れないための保険)
//   3. `Bundle(for:)`経由 (Xcodeでの組み込みなど、実行バイナリ自身の所属バンドルから辿るケース)
//   4. どれも見つからなければ`Bundle.main`にフォールバックする(=ローカライズ文字列は英語キーの
//      ままになるだけで、アプリはクラッシュしない)
private final class VoicewriterKeyboardShortcutsBundleToken {}

extension Bundle {
	static let voicewriterKeyboardShortcutsResources: Bundle = {
		let resourceBundleName = "KeyboardShortcuts_KeyboardShortcuts.bundle"
		...
		let candidateURLs: [URL?] = [
			...
			Bundle(for: VoicewriterKeyboardShortcutsBundleToken.self)
				.resourceURL?
